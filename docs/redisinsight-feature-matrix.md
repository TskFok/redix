# RedisInsight 非 Cloud 功能对比（2026-08-31）

参照本机 `/Users/ushopal/workspace/myself/RedisInsight` 源码，目标仓库未修改。
本轮对比基线：Redix `c0589f0`，RedisInsight `48ee19fab`；本轮改动保留在当前 `main` 工作区。
...
| 普通键浏览、新增/改名/删除/TTL、导入导出 | 基础支持 | 保持 SCAN；大集合字段级分页/增量编辑、批量后台任务仍有差异 | `api/src/modules/browser`、`bulk-actions` |
...
| Stream Group/消费者/Pending | 基础支持 | 本轮增加显式 XCLAIM；实时读取、完整分页及高级 Claim 选项尚有差异 | `api/src/modules/browser/stream` |
| RedisSearch 索引与查询 | 已支持受限 NOCONTENT | 本轮增加字段内容结果和 RESP3 兼容；FT.AGGREGATE、查询构建器/向量索引高级流程仍有差异 | `api/src/modules/browser/redisearch`、`ui/src/pages/vector-search` |
...
| 实例概览 / 数据库分析 | 基础支持 | 仍缺完整时间序列图、分析历史对比和推荐引擎 | `api/src/modules/database-analysis`、`database-recommendation` |
...
|---|---|
| 前端回归 | 28 文件、232 项通过 |
| Rust 常规回归 | 194 项通过，21 项依赖本机服务的用例默认 ignored |
| 隔离普通 Redis | 5 项实际流程通过；3 项 Redis Stack 流程因未配置环境明确跳过 |
