...
function StreamEntriesScope({ connectionId, streamKey, disabled = false, onChanged, onBusyChange }: StreamEntriesProps) {
  ...
  async function mutate(kind: "add" | "delete") {
    ...
    if (!window.confirm(kind === "add" ? `确认向 Stream「${streamKey}」添加消息？` : `确认删除 Stream「${streamKey}」选中的 ${selected.length} 条消息？此操作不可撤销，已有 Pending 引用不会被自动确认。`)) return;
  ...
  return <section className="stream-groups-panel" aria-label="Stream 消息">
    <div className="stream-groups-header">
      <h3>Stream 消息</h3>
