        ...
        <div className="module-tab-content">
          <form className="module-action-card vector-add-card" onSubmit={(event) => void handleAddElement(event)}>
            ...
            <div className="module-form-grid">
              <label className="field"><span>名称</span><input autoCapitalize="off" autoCorrect="off" value={addName} onChange={(event) => setAddName(event.target.value)} disabled={busy || disabled} /></label>
              <label className="field"><span>向量 JSON</span><input autoCapitalize="off" autoCorrect="off" value={addVectorText} onChange={(event) => setAddVectorText(event.target.value)} disabled={busy || disabled} /></label>
            </div>
            <label className="field"><span>属性 JSON（可选）</span><input autoCapitalize="off" autoCorrect="off" value={addAttributesText} onChange={(event) => setAddAttributesText(event.target.value)} disabled={busy || disabled} placeholder='{"label":"demo"}' /></label>
          ...
          {selectedDetails ? (
            <section className="module-element-panel" aria-labelledby="vector-element-title">
              ...
              <label className="field"><span>属性 JSON</span><textarea autoCapitalize="off" autoCorrect="off" value={attributesDraft} onChange={(event) => setAttributesDraft(event.target.value)} disabled={busy || disabled} rows={5} spellCheck={false} /></label>
        ...
        <div className="module-tab-content">
          <form className="module-query-form" onSubmit={(event) => void handleSearch(event)}>
            <div className="module-form-grid module-form-grid-wide">
              ...
              {queryMode === "element" ? <label className="field"><span>元素名称</span><input autoCapitalize="off" autoCorrect="off" value={queryElement} onChange={(event) => setQueryElement(event.target.value)} disabled={busy || disabled} /></label> : <label className="field"><span>向量 JSON</span><input autoCapitalize="off" autoCorrect="off" value={queryVectorText} onChange={(event) => setQueryVectorText(event.target.value)} disabled={busy || disabled} /></label>}
              <label className="field"><span>返回数量</span><input autoCapitalize="off" autoCorrect="off" type="number" min="1" max="200" value={queryCount} onChange={(event) => setQueryCount(event.target.value)} disabled={busy || disabled} /></label>
            </div>
            <div className="module-checkbox-row"><label><input autoCapitalize="off" autoCorrect="off" type="checkbox" checked={queryWithAttributes} onChange={(event) => setQueryWithAttributes(event.target.checked)} disabled={busy || disabled} />返回属性</label><button type="submit" className="button button-primary" disabled={busy || disabled}>执行搜索</button></div>
          </form>
          {state.similarity ? (
            <div className="module-table-wrap"><table className="module-table browser-action-table"><caption>相似度结果</caption><thead><tr><th scope="col">元素</th><th scope="col">距离</th><th scope="col">属性</th><th scope="col">操作</th></tr></thead><tbody>{state.similarity.matches.map((match) => <tr key={match.name}><td><code>{match.name}</code></td><td><code>{Number.isFinite(match.score) ? match.score.toFixed(6) : "—"}</code></td><td><code className="module-value">{compactJson(match.attributes)}</code></td><td><button type="button" className="button button-quiet button-compact" onClick={() => void handleSelectElement(match.name)} disabled={busy || disabled}>查看</button></td></tr>)}</tbody></table></div>
