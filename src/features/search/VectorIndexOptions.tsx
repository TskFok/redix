...
export function VectorIndexOptions({ value, index, disabled, onChange }: {
  ...
  return <fieldset className="editor-fieldset search-vector-options" disabled={disabled}>
    ...
    <div className="form-grid">
      ...
      <label className="field"><span>维度（1–32768）</span><input aria-label={`${label} 向量维度`} type="number" min="1" max="32768" step="1" value={value.dimension || ""} onChange={(event) => onChange({ ...value, dimension: Number(event.target.value) })} /></label>
